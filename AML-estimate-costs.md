...
(2021/5/26 修正) Erterprise エディションは使用されなくなりましたので記載を削除しました。。  
...
(2021/5/26 修正) 以下注意書きにあるコンピューティング インスタンス停止時にも発生する課金は現時点で発生いたしません。起動している時間のみ発生いたします。  
...
例 (東日本リージョン):  
- スタンダード ロード バランサー (約 1.764 [円/時間])
- スタンダード (静的) パブリック IP アドレス (約 0.353 [円/時間])
- マネージド ディスク (VM サイズや利用状況に依存 例: Standard_D11_v2 作成直後で約 0.178 [円/時間])
...
(関連サービス) 
`(1.764 + 0.353 + 0.178 [円/時間])  x 10 [時間/日] x 20 [日] = 459 [円]`  

→ 合計 `5129.6 + 459 = 5588.6 [円]`  
